...
track_outlines_s8 = foreach(i = seq_len(nrow(track_outlines_s8_w1)), .combine = "rbind", .options.future = list(packages = c('tidyverse'))) %dofuture% {
...
# Determine location on track in parallel
tracking_data_s9a = foreach(i = seq_len(nrow(tracking_data_nested)), .combine = "rbind", .options.future = list(packages = c('tidyverse'))) %dofuture% {
...
tracking_data_s9_temp = foreach(i = seq_len(nrow(tracking_data_nested2)), .combine = "bind_rows", .export = "draft", .options.future = list(packages = c('tidyverse'))) %dofuture% {
...
#   geom_point(aes(x = paste(track_id, course_type), y = race_loc, colour = stage)) +
...
tracking_data_s14 = tracking_data_s13 %>%
  ...
  mutate(
    ...
    prop_energy_saved_x_hs = prop_energy_saved * home_stretch
  )
