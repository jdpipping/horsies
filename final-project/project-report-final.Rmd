...
### Motivating and Implementing STRAIN
...
### Clustering Race Horse Movements
...
```{r bar-injury, fig.cap = "Comparison Between Racing More or Less than Expected for Horses who were Severely Injured in 2019"}
model_resids |>
  cbind(nb_model$fitted.values) |>
  mutate(under_raced = if_else(.std.resid < -1, TRUE, FALSE),
...
In our project, we found that a substantial proportion of horses that raced less than expected given their age and the calendar year. Through modeling expected race counts for under-racing, we were able to generate more injury signal than available by just looking at severe horse injuries in 2019. This, in addition to our analysis of how horse's race, could provide some insight into potential patterns and factors leading to horse injures that would be interesting to investigate further, particularly with respect to speed and acceleration. These data-driven insights on the association between certain movement profiles and horse injuries could allow for our external partners at the NYRA improve race horse welfare.
...
* Collecting more start information to close the gap between the number of horses tracked in 2019 and the number of horses analyzed for under-racing to generate more injury signal.
