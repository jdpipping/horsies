...
horse_profiles |>
  group_by(if_injury_reported, ever_under_raced) |>
  count() |>
  knitr::kable()
```
...
p1 <- model_resids|> 
  ...
  theme(text = element_text(face = "bold", size = 8), axis.title = element_text(size = 10),
...
model_resids |>
  cbind(nb_model$fitted.values) |>
  mutate(under_raced = if_else(.std.resid < -1, TRUE, FALSE),
         over_expected = if_else(.std.resid > 0, "Over Expected Count", "Under Expected Count")) |>
  mutate(over_expected = fct_relevel(over_expected, "Under Expected Count", "Over Expected Count")) |>
  rename(.fitted = `nb_model$fitted.values`) |>
  left_join(horse_profiles_injured, by = "horse_id") |>
